...
* This is a fork
This is a forked repository of original oxwm repo by Tony
https://github.com/tonybanters/oxwm

*Caution:* Please read this section carefully first before deciding to use this fork.

** Branches
This fork organizes its changes into separate feature branches for
clean upstream contribution:

*** =fix/keybind-focus-move-args=
Keybind argument forwarding. Previously =FocusStack=, =MoveStack=,
=IncMaster=, =ViewNextTag=, =ScrollLeft=, and =ScrollRight= all
ignored the =int_arg= passed from the Lua config, hardcoding direction
values of 1 or -1. Now these actions use =int_arg= when non-zero,
falling back to the default direction. Also adds the =FocusPrev= action
alias to the Lua config parser.

*** =fix/disable-window-cycling=
Removes window list wrap-around in =focusstack=. Focusing past the last
visible window no longer jumps to the first, and focusing before the
first no longer jumps to the last. The end of the client list acts as a
hard boundary.

*** =fix/zombie-processes=
Prevents zombie child processes. Spawned processes (via
=spawnCommand=, =spawnTerminal=, and autostart) use =fork=/=exec= but
the parent never reaped them, leaving =<defunct>= entries in the
process table. Sets =SIGCHLD= to =SIG_IGN= before the event loop so
the kernel auto-reaps exited children.

*** =fix/smooth-scroll-and-viewport-check=
Combines scroll animation smoothness improvements with viewport-aware
scrolling behavior:

- *Smooth animation:* Replaces =XSync= with non-blocking =XFlush=,
  adds =resizeClientAnim= for animation frames, inserts mid-burst
  animation ticks (every 8 events), reduces poll timeout to 6ms,
  and suppresses =EnterNotify= / =FocusIn= / =MotionNotify= /
  =PropertyNotify= during active animation to prevent focus stealing.

- *Viewport-aware scrolling:* =getTargetScrollForWindow= now checks
  whether the target window is already fully visible and returns the
  current offset if so, avoiding unnecessary rescrolls. New windows
  scroll to their position instead of resetting to offset 0.
  Closing a floating window no longer triggers scroll recalculation.

- *Bar performance:* Caches =XftColor= allocations per bar instead of
  alloc/free per =drawText=. Shell blocks spawn child processes
