...
> [!IMPORTANT]
> Please read this section before deciding to use this

** Fixes
- Scrolling animations to not lock at 60fps
- Disabled cycling of windows, on my fork the last is last and it
  doesnt jump to the first if you go past
- Scrolling doesnt always try to force the focused
  window on the left so we get rid of unecessary scrolling
- Smoother scrolling for heavy apps like web browsers (Still not perfect)
- Fixed focusing keybinds, not changing focused window actually works
  properly

All the implementations are mainly AI generated with human supervision,
so keep that in mind when deciding if you want to use this fork.
